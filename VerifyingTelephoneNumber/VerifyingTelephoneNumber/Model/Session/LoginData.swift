...
class LoginData {
    ...
    // MARK: Log in aria -
    let countryDictionary  = ["UA":"380","AF":"93","AL":"355","DZ":"213","US":"1",
...
//                              "TL":"670","VE":"58","VN":"84","VG":"284","VI":"340"
    ...
    @Published var IDCode: String?
    
    // User load status
    @Published var successAuth = false
    @Published var failAuth = false
    @Published var blockedAccount = false
    @Published var numberOfAttempt = ConstantsGlobal().numberOfAttempt
    
    // Loading view
    @Published var loadingProcess = false
    private var number: String?
    @Published var showWarningAlert = false
    @Published var IDCodeIsNil: Bool = false
    
    private func handleError(_ error: Error?) {
        if let error = error {
            self.errorMSG = error.localizedDescription
            print("errorMSG", self.errorMSG)
...
extension LoginData {
    ...
    func placeholderNumber(countryName: String) -> String {
        var placeholderNumber = ""
        switch countryName {
        case "Afghanistan":
            placeholderNumber = "123456789"
        case "Albania":
            placeholderNumber = "123456789"
        case "Algeria":
            placeholderNumber = "123456789"
        case "Ukraine":
            placeholderNumber = "123456789"
        case "United States":
            placeholderNumber = "1234567891"
        default:
            placeholderNumber = "000000000"
        }
    ...
    func sendCodeTo(number: String) {
        PhoneAuthProvider.provider().verifyPhoneNumber(number, uiDelegate: nil) { IDCode, error in
            if let error = error {
                print(error.localizedDescription)
                return
            }
            
            guard let IDCode = IDCode else {
                print("IDCode is empty")
                withAnimation {
...
extension LoginData {
    ...
    func verifyWith(otp: String) {
        ...
        let credential = PhoneAuthProvider.provider().credential(withVerificationID: IDCode, verificationCode: otp)
        ...
        Auth.auth().signIn(with: credential) { result, error in
            ...
            case .loggedOut:
                if self.numberOfAttempt > 1 {
                    self.numberOfAttempt -= 1
                    withAnimation { self.failAuth = true }
                } else if self.numberOfAttempt == 1 {
                    self.numberOfAttempt -= 1
                    withAnimation {
                        self.failAuth = false
                        self.blockedAccount = true
                    }
                }
            }
        }
    ...
    func requestCodeOn(number: String) {
        print("Try Code send")
        withAnimation {
            self.loadingProcess = true
        }
        
        sendCodeTo(number: number)
        withAnimation {
